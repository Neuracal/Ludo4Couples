...
也可以在网页右上角点击“设置”，直接编辑“自定义任务”文本框。这个文本框初始内容就是 `src/tasks.md` 的任务列表，打开设置时会自动全选，方便直接粘贴替换。点击“保存”后，游戏会使用你输入的任务重新生成棋盘并重新洗牌。自定义任务只在当前页面会话中生效；刷新网页后会恢复为程序内置的 `src/tasks.md` 内容。
